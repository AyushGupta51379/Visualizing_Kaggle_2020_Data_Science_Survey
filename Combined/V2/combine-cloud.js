...
render2 = (myWords, types) => {
    ...
    var borderPath2 = svg2.append("rect")
        ...
        .attr("width", width2 + margin2.left)
        .style("stroke", 'black')
        .style("fill", "none")
        .style("stroke-width", 0.5);
    // header
    heading = svg2.append("text")
        .attr("x", 1.2 * margin2.left)
        .attr("y", 1.5 * margin2.top)
        .text("Word cloud")
        .attr("font-size", height2 * 24 / 450)
        .attr("font-weight", 750)

    // footer
    footer = svg2.append("text")
        .attr("x", 1.2 * margin2.left)
        .attr("y", height2 - 0.5 * margin2.top)
        .text("Most used " + types)
        .attr("font-size", height2 * 23 / 450)
    ...
    var layout = d3.layout.cloud()
        .size([width2, height2])
        .words(myWords.map(function (d, i) { return { text: d.word, size_: Math.round((Math.sqrt(d.size) / 1.8 + 1.5)*.8), real_size: d.size }; }))
        .padding(5)        //space between words
        .rotate(function () { return ~~(Math.random() * 2) * 90; })
        .fontSize(function (d,i) { return d.size_; })      // font size of words
        .on("end", draw);
    layout.start();
    ...
    function draw(words) {
        w = svg2
            ...
            .data(words)
            .enter().append("text")
            .attr("font-size", function (d, i) { return d.size_; })
        ...
        title = w.append("title")
            .text(function (d, i) { return d.text + ", "+ d.real_size + " respondents"; })
    }
}
